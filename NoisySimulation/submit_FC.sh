#!/bin/bash
# Opt-FC-TFI evolution-time-sweep submission script for CURC Alpine.
#
...
if [[ -z "${CODE_DIR:-}" ]]; then
  for candidate in "${REPO_ROOT}/Classical" "${REPO_ROOT}" "${REPO_ROOT}/.."; do
    if [[ -f "${candidate}/noisy_qrc_slurm.py" ]]; then
      CODE_DIR="${candidate}"
      break
    fi
  done
...
if [[ -z "${CODE_DIR:-}" || ! -f "${CODE_DIR}/NN-or-FC-NOISY-SLURM.py" ]]; then
  echo "FATAL: could not find noisy_qrc_slurm.py near ${REPO_ROOT}; set CODE_DIR." >&2
  exit 1
fi
echo "CODE_DIR=${CODE_DIR}"

OUTPUT_DIR="${OUTPUT_DIR:-${CODE_DIR}/results/exp_fc}"
...
echo "==============================================================="
echo "Experiment        : FC-TFI evolution-time sweep"
