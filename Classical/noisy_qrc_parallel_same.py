...
We are computing two circuits in parallel, halfing the number of shots from 4096 to 2048 using parallel circuit repetion (6->12 qubits)
...
from qiskit_aer.noise import NoiseModel, ReadoutError, coherent_unitary_error, pauli_error
...
class CorrelatedNoiseSpec:
    ...
    apply_to_pairs: str = "coupling"

    @property
    def is_identity(self) -> bool:
        return (self.p_zz_after_2q <= 0.0 and self.alpha_zz_coherent <= 0.0 and self.readout_corr_strength <= 0.0)
    ...
    def label(self) -> str:
        if self.is_identity:
            ...
            return f"pzz{self.p_zz_after_2q:g}_azz{self.alpha_zz_coherent:g}_rc{self.readout_corr_strength:g}_{self.apply_to_pairs}"
...
def _resolve_corr_pairs(backend: FakeBackendV2, n_qubits: int, mode: str) -> List[Tuple[int, int]]:
    """Return (a, b) qubit pairs that correlated channels are attached to based on backend or ring as fallback."""
    if mode == "coupling":
        ...
        for (a, b) in raw:
            if a >= n_qubits or b >= n_qubits: # we don't care
                continue
...
def _correlated_readout_error(p_flip: float, c:float) -> ReadoutError:
    """
    Two qubit assignment with correlation strength in [0, 1]

    c=0 is two indepedent per-qubit flaips at rate p_fip

    c=1 sends all o fthe flip mass to the joint 00<->11 / 01<->10 channel.
    """
    pf = p_flip * (1.0 - c)
    pc = p_flip * c
    ...
    #prevent drift before Aer normalizes
    M = np.clip(M, 0.0, 1.0)
    M = M / M.sum(axis=1, keepdims=True)
    return ReadoutError(M)

def augment_noise_model_with_correlation(
        ...
        for (a, b) in pairs:
            for g in two_qubit_gates:
                noise_model.add_quantum_error(
                    zz_err, [g], [a, b], warnings=False,
                )

    if spec.alpha_zz_coherent > 0.0:
        coh_err = coherent_unitary_error(
            Operator(RZZGate(spec.alpha_zz_coherent)).data
        )
        for (a, b) in pairs:
            for g in two_qubit_gates:
    ...
    if spec.readout_corr_strength > 0.0:
        ro_err = _correlated_readout_error(spec.p_flip_readout, spec.readout_corr_strength)
        for (a, b) in pairs:
...
def build_noise_specs(args: argparse.Namespace) -> List[CorrelatedNoiseSpec]:
    if args.noise_sweep:
        sweep_vals = [
            float(tok.strip()) for tok in args.noise_sweep.split(",")
            if tok.strip()
        ]
        if not sweep_vals:
            raise ValueError("--noise-sweep needs values")
        return [
            CorrelatedNoiseSpec(
                p_zz_after_2q=v,
